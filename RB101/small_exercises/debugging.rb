...
def move(n, from_array, to_array)
  size_start = from_array.size
  to_array << from_array.shift
  if from_array.size != size_start - n
...
def neutralize(sentence)
  ...
  words_size.times do |x|
    words.delete(words[x]) && redo if negative?(words[x])
  end
...
def idf(term, documents)
  ...
  Math.log(number_of_documents.to_f / number_of_documents_with_term).round(1)
end
